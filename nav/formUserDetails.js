...
const  generateCard = async (packageData) => {
  displayColors();
    ...
    //  console.log(packageStatus);
      ...
      <hr/>
      <p class="flex-package"><span>Package status:</span><span id="own_parents" class="own_status">${packageStatus}</span></p>
      <hr/>
      ...
      edit
          ...
          ${packageData.cancel_status === "cancelled" ? `<button type="submit" id="btn_cancels"  class="btn red-color float-end btn-cancel" data-loading-text="Loading..."  data-package-id="${packageData.id}" data-package-number="${packageData.package_number}" disabled><span class="material-symbols-outlined">
          cancel
          </span>cancel</button>` : `<button type="submit" id="btn_cancels"  class="btn red-color float-end btn-cancel" data-loading-text="Loading..."  data-package-id="${packageData.id}" data-package-number="${packageData.package_number}"><span class="material-symbols-outlined">
          cancel
          </span>cancel</button>`}
    </div>
      `;
...
const swalCancel = (e,packageNumber) =>{
  e.preventDefault();
  const packageIds = e.target.dataset.packageId;
  const cancelStatus = "cancelled";
  console.log(packageIds);
    new swal({
        title: `Cancel order ${packageNumber}`,
        html: `
     <div class="form-group hide-overflow row cancel">
<div class="forms-child">
    <input class="form-control dispatch-date text-3 h-auto py-2 input" placeholder="Enter the reason for cancellation"  type="text"  name="cancel_reason" id="cancel_reason">
</div>
</div>
     `,
        ...
        preConfirm: async () => {
          ...
          const cancelledReason= document.getElementById("cancel_reason").value;
          ...
          editData.append("cancel_reason",cancelledReason);
          editData.append("package_id",packageIds);
          editData.append("cancel_status",cancelStatus);

          console.log([...editData]);
            // sending edited data to the database
              try {
            const response = await fetch(`https://api5.staging.iguru.co.ke/api/cancel`, {
              method: "POST",
              headers: {
                'Content-Type': 'application/json',
              },
     ...
     .then(result => {
        if(result.isConfirmed) {
            new swal("Order Cancelled!", "Your order has been cancelled successfully.", "success");
            setTimeout(() => {
              location.reload();
            },2000)
        }
...
document.getElementById("cardscontainer").addEventListener("click",(e)=>{
  e.preventDefault();
  const packageNumber = e.target.dataset.packageNumber;
  if(e.target.classList.contains("btn-cancel")){
      swalCancel(e,packageNumber);
    }
})
...
const disableButton = () =>{
  const packageStatusCheck = document.querySelectorAll(".own_status");

      //  cancelledButton.forEach((buttonItem) => {
       packageStatusCheck.forEach((cancelStatusItem) => {
        console.log(cancelStatusItem.textContent);
        if(cancelStatusItem.textContent === "Cancelled"){
        let buttonItem = document.getElementById("btn_cancels");
            buttonItem.disabled = true;
        }else if(cancelStatusItem.textContent === "Collected"){
        let buttonItem = document.getElementById("btn_cancels");
         buttonItem.disabled = false;
        }else if(cancelStatusItem.textContent === "Confirmed"){
        let buttonItem = document.getElementById("btn_cancels");
          buttonItem.disabled = false;
        }else if(cancelStatusItem.textContent === "Dispatched"){
        let buttonItem = document.getElementById("btn_cancels");
         buttonItem.disabled = false;
        }else if(cancelStatusItem.textContent === "Completed"){
        let buttonItem = document.getElementById("btn_cancels");
          buttonItem.disabled = false;
        }else if(cancelStatusItem.textContent === "Returned"){
        let buttonItem = document.getElementById("btn_cancels");
          buttonItem.disabled = false;
        }else if(cancelStatusItem.textContent === "New"){
        let buttonItem = document.getElementById("btn_cancels");
          buttonItem.disabled = false;
        }
      // })
