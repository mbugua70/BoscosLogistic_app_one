...
async function fetchPackageData(customerAccountId) {
 const starting_time = Date.now();
    try{
        const response = await fetch(`https://api5.staging.iguru.co.ke/api/collected/${customerAccountId}`, {
            method:"GET",
            headers:{
                'Content-Type':'application/json',
              }
...
async function displayPackages() {
    ...
    if(packageDatashow.data.length === 0){
      const cardContainer = document.getElementById("cardscontainer");
      const message = `<p> You don't have  collected order</p>`;
      cardContainer.innerHTML = message;
    }else{

      packageDatashow.data.forEach(async (packageType)=>{
        if(packageType.verification_status === "verified" && packageType.collection_status === "collected" &&  packageType.cancel_status !== "cancelled" && packageType.dispatch_status !== "dispatched" && packageType.returned_status !== "returned" && packageType.confirmation_status !== "confirmed"){
